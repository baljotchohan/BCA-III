 ...
 * Supports Student Mode and Authenticated Admin Mode.
 * Transport: Standard Input/Output (stdio) & Streamable HTTP (JSON-RPC 2.0)
...
// Helper: Normalize Subject ID / Name
function normalizeSubjectId(input) {
  ...
  if (s.includes("machine") || s.includes("learning") || s.includes("ml")) return "machine-learning";
...
const PUBLIC_TOOLS = [
  {
    name: "get_syllabus",
    description: "Get the complete official Panjab University BCA 3rd Sem syllabus for a specific subject or all subjects.",
    inputSchema: {
      type: "object",
      properties: {
        subject_id: {
  ...
  },
  {
    ...
    inputSchema: {
      ...
      properties: {
        query: {
          type: "string",
          description: "Search query or keyword (e.g. '8086', 'bisection', 'binary search tree', 'pipeline', 'Newton Raphson')",
          default: ""
        },
        subject_id: {
          type: "string",
  ...
  },
  {
    ...
    inputSchema: {
      ...
      properties: {
        id: {
          type: "string",
          description: "The note ID or Firebase key (e.g. '-O5...', 'custom-note-17...')"
        }
      },
      required: ["id"]
    }
...
// Admin & Authoring Tools (Unlocked with Passkey / Bearer Auth)
const ADMIN_TOOLS = [
  {
...
const PROMPTS = [
  ...
  {
    name: "daily_lecture_logger",
    description: "Draft and publish a daily classroom lecture log with summary and homework.",
    arguments: [
      { name: "subject_id", description: "Subject ID", required: true },
      { name: "topic", description: "Class topic covered", required: true },
      { name: "notes", description: "Key takeaways or assigned code", required: true }
    ]
  }
];
...
// Helper: Put / Patch JSON to Firebase RTDB
...
async function handleMcpRpc(payload, authHeader = '') {
    ...
    return {
      ...
      result: {
        ...
        instructions: isAdmin
          ? "ADMIN MODE ACTIVE: Full permissions granted to create/publish notes under Baljot Chohan's name, record classroom lectures, broadcast announcements, and manage cloud data."
  ...
  if (method === "tools/call") {
    ...
    if (name === "get_unit_details") {
      ...
      if (!sub) {
        return {
          ...
          id: reqId,
          result: {
            content: [{ type: "text", text: `Subject '${args.subject_id}' not found. Available subjects: ${Object.keys(SYLLABUS_INDEX).join(", ")}` }],
            isError: true
          }
        };
      }
    ...
    if (name === "get_digital_notes" || name === "list_digital_notes" || name === "get_notes" || name === "list_notes") {
      ...
      // Extract legacy notes in lectures
    ...
    // --- TOOL: get_syllabus_structure_for_ai ---
